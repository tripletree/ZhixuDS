Modal on the brand's overlay recipe: ink-950/80 + 4px blur backdrop, frosted `.glass` card (1rem radius, 32px padding) entering with the 0.3s scale/rise `fm-dialog-in` animation. Escape and overlay click both close. `footer` renders a right-aligned action row below the body. For long content (picker grids) pass `scrollBody` to make the body scroll — it clips floating panels like Listbox, so leave it off for forms.
...
<Dialog open={open} onClose={close} align="left" size="lg" scrollBody title="品牌"
  footer={…取消/确定…}>
