...
4. React mirror (for HTML prototypes/mocks): `components/core/*.jsx` + `components/ui/*.jsx` (same app-level component set as the Vue package).
5. Showcase site: the Vue package doubles as the deployable demo site (`npm run dev`, or Vercel via root `vercel.json`) — landing at `#/`, component gallery at `#/components`, foundations at `#/guidelines`.
