...
- **Data-viz**: thin 1.4–1.6px strokes; azure primary series, champagne/mist/rouge secondary at 0.6 opacity; radar polygons with 0.18 fill; gradient progress bars azure-deep→azure. Full charts (ECharts) use the categorical `--chart-1…8` palette (azure first, muted indigo/sage/champagne/rouge/clay/steel/mist), flat-topped bars, hairline grid, `.fm-float` tooltips, and a Vue-rendered legend with 全选/反选 pills.
