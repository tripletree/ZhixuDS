...
- **Buttons**: pill-shaped always. Primary = bone-filled, ink text; secondary/ghost = hairline `border-bone/15` + transparent; icon button = 36px circle. All hairline buttons share one hover/focus state in both themes: border `azure/40`, fill `azure/10`, text and icon `azure-bright` together, plus a 3px `azure/12` focus ring.
